...
def johnson(n, S):
    ...
    for u, v, w in S:
        Adj[u].append(v)
        W[(u, v)] = w
    w = lambda u, v: W.get((u, v), INF)
    Adj.append([v for v in range(n)])   # add a super node n
    for v in range(v):
        W[(n, v)] = 0   # 0-weight edges from n
    output = bellman_ford(Adj, w, n) # from n
    if output is None:   # contains negative-weight cycle
        return None
    d = output[0]
    # then we build G' and run dijkstra
    w = lambda u, v: W[(u, v)] + d[u] - d[v] if (u, v) in W else INF
    for u in range(n):
        du, _ = dijkstra(Adj, w, u)
        for v in range(n):
            D[u][v] = INF if du[v] == INF else du[v] - d[u] + d[v]
